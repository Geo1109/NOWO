  ...
  return (
    <div className="fixed top-24 right-6 z-40 flex flex-col items-end gap-3">
      <button 
        ...
        className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-xl transition-all duration-300 ${active ? 'bg-danger text-white animate-pulse' : 'glass text-danger border-danger/20'}`}
      >
        <Shield size={24} />
      </button>
...
export default function App() {
  ...
  const [safeSpaces] = useState<SafeSpace[]>(MOCK_SAFE_SPACES);
  ...
  useEffect(() => {
    ...
    mapRef.current.eachLayer((layer: any) => {
      if (layer instanceof L.Circle || (layer instanceof L.Marker && layer !== userMarkerRef.current)) {
    ...
    if (showSafeSpaces) {
      safeSpaces.forEach(space => {
        ...
        const marker = L.marker([space.lat, space.lng], { icon }).addTo(mapRef.current);
    ...
    if (reportLocation) {
      ...
      L.marker(reportLocation, { icon }).addTo(mapRef.current);
    }
  }, [reports, showSafeSpaces, reportLocation]);

  const handleReportSubmit = (categories: string[], details: string, isLive: boolean) => {
      ...
      <AnimatePresence>
        ...
        {selectedZone && (
          <ZoneDetails 
            ...
            onConfirm={() => {
              setReports(reports.map(r => r.id === selectedZone.id ? { ...r, weight: r.weight + 1 } : r));
...
const RoutePlannerScreen = ({ onClose, t, userLocation, reports, onDrawRoute, onStartNav }: { key?: string, onClose: () => void, t: any, userLocation: [number, number], reports: Report[], onDrawRoute: (r: RouteInfo) => void, onStartNav: () => void }) => {
  ...
  const findRoutes = async (dest: SearchResult) => {
    ...
    try {
      ...
      // Fetch alternatives
      const url = `https://router.project-osrm.org/route/v1/foot/${userLocation[1]},${userLocation[0]};${destLng},${destLat}?overview=full&geometries=geojson&alternatives=true&steps=true`;
      
      const response = await fetch(url);
      const data = await response.json();

      if (data.code === 'Ok') {
        const processedRoutes: RouteInfo[] = data.routes.map((r: any, idx: number) => {
          // Calculate safety score based on proximity to reports
          let safetyScore = 100;
        ...
        if (processedRoutes.length > 1) {
          processedRoutes[0].type = 'safe';
          processedRoutes[1].type = 'fastest';
        }

        setRoutes(processedRoutes);
        if (processedRoutes.length > 0) {
          onDrawRoute(processedRoutes[0]);
        }
      }
    } catch (error) {
            ...
            {routes.map((route, idx) => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className={`text-xl font-black ${route.type === 'safe' ? 'text-safe' : 'text-slate-900'}`}>
                      {route.type === 'safe' ? t.safePath : t.fastestPath}
                    </h3>
                    <p className="text-xs text-slate-400 font-bold">
                      {route.safetyScore > 90 ? "Maximum safety" : `Safety Score: ${route.safetyScore}%`}
                    </p>
                  </div>
                  {route.type === 'safe' && (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-8 flex flex-col gap-8 pb-32">
        {!reportLocation ? (
          <div className="p-6 bg-rose-50 rounded-3xl border border-rose-100 flex items-center gap-4">
            ...
            <p className="text-sm font-bold text-rose-600">Tap on the map to mark the location</p>
          </div>
        ) : (
        ...
        <div className="flex flex-wrap gap-3">
          {categories.map(cat => (
            <button 
              ...
              className={`px-5 py-4 rounded-2xl text-xs font-bold transition-all ${selected.includes(cat.id) ? 'bg-rose-500 text-white glow-accent scale-105' : 'glass text-slate-500'}`}
            >
              {cat.label}
            </button>
          ))}
        ...
        <textarea 
          ...
          className="w-full h-40 glass rounded-3xl p-6 text-sm font-bold focus:outline-none resize-none shadow-inner text-slate-900"
        />
        ...
        <div className="flex items-center justify-between p-6 glass rounded-3xl">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${isLive ? 'bg-rose-500 text-white shadow-lg shadow-rose-200' : 'bg-slate-100 text-slate-400'}`}
            >
              {t.yes}
            </button>
            <button 
              onClick={() => setIsLive(false)}
              className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${!isLive ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-400'}`}
            >
              {t.earlierToday}
            </button>
          </div>
      ...
      <div className="p-8 border-t border-slate-200 bg-slate-50">
        <button 
          ...
          className="w-full h-16 bg-rose-500 text-white rounded-2xl font-black text-lg glow-accent disabled:opacity-50 disabled:glow-none active:scale-95 transition-all"
        >
          {t.reportZone}
        </button>
      </div>
...
const SpaceDetails = ({ space, onClose, t }: { space: SafeSpace, onClose: () => void, t: any }) => {
  const iconMap: any = {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-xs text-slate-400 font-bold">0.4 km {t.distance}</p>
            </div>
          </div>
        </div>
        <button onClick={onClose} className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400">
