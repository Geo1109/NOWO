...
export const AuthScreen = ({ onSuccess, onClose }: AuthScreenProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'register') {
        ...
        await setDoc(doc(db, 'users', cred.user.uid), { uid: cred.user.uid, name, email: email.toLowerCase(), createdAt: serverTimestamp(), emergencyContacts: [], incomingAlerts: [] });
        await sendEmailVerification(cred.user);
        await saveTokenToFirestore(cred.user.uid);
        setMode('verify');
      } else {
        const cred = await signInWithEmailAndPassword(auth, email, password);
        if (!cred.user.emailVerified) { setMode('verify'); return; }
        await saveTokenToFirestore(cred.user.uid);
        onSuccess();
      }
    } catch (e: any) {
  ...
  const handleForgotPassword = async () => {
    ...
    if (!email.trim()) { setError('Introdu email-ul pentru a reseta parola.'); return; }
    setLoading(true);
    try {
      ...
      setError(messages[e.code] || 'Nu am putut trimite email-ul de resetare.');
    } finally { setLoading(false); }
  };

  const checkVerified = async () => {
    const user = auth.currentUser; if (!user) return;
    setLoading(true); await user.reload();
    if (user.emailVerified) { await saveTokenToFirestore(user.uid); onSuccess(); }
    else setError('Email-ul nu a fost verificat încă.');
    setLoading(false);
  };

  const resendVerification = async () => {
    const user = auth.currentUser; if (!user || resendCooldown > 0) return;
  ...
  if (mode === 'reset') {
    return (
      <div className="fixed inset-0 z-[200] flex flex-col bg-white">
        {onClose && (
          <button onClick={onClose} className="absolute top-14 right-6 w-10 h-10 rounded-2xl flex items-center justify-center z-10 bg-slate-100">
            <X size={18} className="text-slate-500" />
          </button>
        )}
        <div className="relative overflow-hidden flex flex-col items-center justify-end pb-8 pt-20"
          ...
          <div className="relative flex flex-col items-center gap-3">
            ...
            <div className="text-center">
              <h1 className="text-white text-2xl font-bold tracking-tight">Resetare parolă</h1>
              <p className="text-white/50 text-sm mt-0.5">Trimitem un link pe email</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 px-6 pt-6 flex flex-col">
          {resetSent ? (
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <div className="text-center">
                <p className="font-bold text-slate-900 text-lg">Email trimis!</p>
                <p className="text-slate-500 text-sm mt-1">Verifică inbox-ul pentru {email}</p>
              </div>
              <button onClick={() => { setMode('login'); setResetSent(false); }}
                className="w-full h-14 rounded-2xl font-bold text-white flex items-center justify-center gap-2 mt-4"
            ...
            <>
              <p className="text-slate-500 text-sm mb-5">Introdu email-ul contului tău și îți vom trimite un link de resetare a parolei.</p>
              ...
              <button onClick={handleForgotPassword} disabled={loading}
                ...
                style={{ background: loading ? '#93c5fd' : PRIMARY, boxShadow: loading ? 'none' : `0 8px 24px ${PRIMARY}55` }}>
                {loading
                  ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  : <><Send size={18} /> Trimite link de resetare</>}
              </button>
              <button onClick={() => { setMode('login'); setError(''); }}
                className="w-full mt-3 py-3 text-sm font-semibold text-slate-500">
                Înapoi la login
              </button>
            </>
          )}
    ...
    return (
      <div className="fixed inset-0 z-[200] flex flex-col" style={{ background: PRIMARY }}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
          ...
          <button onClick={checkVerified} disabled={loading}
          ...
          <button onClick={resendVerification} disabled={resendCooldown > 0}
